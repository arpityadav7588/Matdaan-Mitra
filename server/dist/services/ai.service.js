...
class AIService {
  ...
  `;
     ...
     */
    static getCachedModel(modelName, systemInstruction) {
    ...
    static async getRelevantContext(query) {
        ...
        try {
            ...
            const factsWithEmbeddings = this.knowledgeBase.filter((f) => !!f.embedding);
            const similarities = factsWithEmbeddings
                .map(f => ({
                ...f,
