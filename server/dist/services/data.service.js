...
class DataService {
    static async getCandidates(constituency) {
        try {
            ...
            const snapshot = await firebase_1.db.collection('candidates').where('constituency', '==', constituency).get();
            if (snapshot.empty) {
    ...
    static async getTimeline() {
        try {
            if (!firebase_1.db)
                throw new Error('DB not initialized');
            const snapshot = await firebase_1.db.collection('timeline').orderBy('date', 'asc').get();
            if (snapshot.empty) {
                return [
